...
\author{Norbert Eke \\
  ...
  3333 University Way \\
  Kelowna, BC Canada \\
  {\tt norbert.eke@alumni.ubc.ca} \\\And
  Abdallah Mohamed \\
  UBC Okanagan \\
  3333 University Way \\
  Kelowna, BC Canada \\
...
\maketitle
\begin{abstract}
  According to the Crimes Against Children Research Center one in five U.S. teenagers who regularly log on to the Internet say they have received an unwanted sexual solicitation via the web. This project’s sole purpose is to design an approach that could help online communities enhance their member’s safety by detecting and classifying conversations to different levels of danger. This research project aims to join the powers of computational linguistics with statistical machine learning to decipher the insight lying in conversations, then make predictions on whether or not a specific conversation should be flagged for containing sexual predatory behaviour. A new approach to sexual predator identification was designed by putting an emphasis on the insight that lies within the contextual details of a conversation. The approach contains a 2 stage classification system, which is highly flexible and customizable to detecting and classifying other dangerous textual data.
\end{abstract}
